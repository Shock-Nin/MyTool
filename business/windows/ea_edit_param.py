            ...
            with open(MQ4_PATH + "/" + file, "r", encoding="utf16") as f:
                ...
                for i in range(0, len(datas)):
                    ...
                    if 0 <= datas[i].find("if ("):
                        ...
                        while (
                            datas[i].find("if (") < 0
                            and datas[i].find("for") < 0
                            and datas[i].find("else {") < 0
                        ):
    ...
    for i in range(0, len(eas)):
        ...
        with open(path, "w") as f:
            for n in range(0, len(eas[i])):
                ...
                if 0 == n:
                    f.write(
                        ...
                        + ",".join(
                            [
                                (
                                    prm[k].split("[")[0].replace("_", "")
                                    + "_0"
                                    + str(
                                        int(
                                            prm[k].split("=")[0][
                                                prm[k].split("=")[0].find("[")
                                                + 1 : prm[k].split("=")[0].find("]")
                                            ]
                                        )
                                        + 1
                                    )
                                    if 0 <= prm[k].split("=")[0].find("[")
                                    else prm[k].split("=")[0].replace("_", "")
                                )
                                .replace("s_", "_")
                                .capitalize()
                ...
                f.write("\n" + prm[0].replace("USD", "") + ",")
                f.write(
                    ",".join([prm[k].split("=")[1] for k in range(1, len(prm) - 1)])
                )
...
def _edit_htmfiles():
    ...
    # .htmファイル名のリストを取得
    htm_lists = glob.glob(
        cst.MT4_DEV[cst.PC] + "MQL4/Files/" + ea_name + "/Test_*/*.htm"
    )
    read_files = []
    err_msg = ""
        ...
        for k in range(0, len(cst.CURRENCIES_EA[0])):
            ...
            for file in htm_lists:
                if file.find("_" + tf[i] + "_") < 0:
                    continue
                if (
                    "GOLD" != cst.CURRENCIES_EA[0][k]
                    and file.find(cst.CURRENCIES_EA[0][k]) < 0
            ...
            for m in range(0, len(read_files)):
                for n in range(0, len(read_files[m])):
                    ...
                    for p in range(0, len(read_files[m][n])):
                        ...
                        if (
                            "GOLD" != cst.CURRENCIES_EA[0][k]
                            and title[0].find(cst.CURRENCIES_EA[0][k]) < 0
                        ) or (
                            "GOLD" == cst.CURRENCIES_EA[0][k]
                            and title[0].find("XAU") < 0
                        ):
                            continue
...
def _output_html():
    ...
    for i in range(0, len(read_sets)):
        ...
        html += (
            '<table><tr><td class="eaName" align="center">'
            + ea_name
            + "</td></tr><tr><td>"
        )
            ...
            if "WheSitDoch" == ea_name:
                ...
                for prm in prms:
                    ...
                    if 0 < len(time_frame) and time_frame != str(prms[prm][key]):
                        ...
                        prm_html += (
                            ' colspan="' + str(count) + '">' + time_frame + "</td>"
                        )
                        count = 0
                ...
                if "TimeFrame" == key:
                    prm_html += '<td class="prm" align="center"'
                    prm_html += (
                        ' colspan="' + str(count + 1) + '">' + time_frame + "</td>"
                    )
                ...
                for prm in prms:
                    ...
                    prm_html += '<td class="prm' + (
                        " zero" if 0 == len(str(prms[prm][key]).strip()) else ""
                    )
                    prm_html += '" align="' + (
                        "center" if "Currency" == key else "right"
                    )
                    prm_html += '">' + str(prms[prm][key]) + "</td>"

                if 0 <= key.find("Currency") or 0 <= key.find("OpenWait"):
                    prm_html += (
                        '<tr><td bgcolor="#CCCCCC" height="5" colspan="'
                        + str(total)
                        + '"></td></tr>'
                    )
                ...
                html += (
                    ...
                    + key
                    + "</td>"
                    + prm_html.replace("USD", "").replace("XAU", "GOLD")
                    + "</tr>"
                )
            else:
                prm_html = ""
                ...
                for k in range(0, len(prms)):
                    ...
                    zero = ""
                    try:
                        zero = " zero" if 0 == float(prms[k][key]) else ""
                    except:
                        pass
                    ...
                    try:
                        ...
                        if 0 < len(set_cur) and set_cur == prms[k]["Comments"]:
                            ...
                            set_html += (
                                '<td class="prm'
                                + ("" if str(prms[k][key]) == item else " ng_set")
                                + (
                                    " up_line"
                                    if 0 <= key.find("Lot") or key in ["Trend4H"]
                                    else ""
                                )
                                + '" align="'
                                + (
                        ...
                        for n in range(0, len(mqls)):
                            ...
                            prm_html += (
                                ...
                                + '" align="'
                                + ("center" if "Comments" == key else "right")
                                + '">'
                                + str(prms[k][key])
                                + "</td>"
                            )
