...
def _edit_unit_list():
    ...
    for ea in ['Unit', 'WheSitDoch']:

        tests = (glob.glob(cst.MT4_DEV[cst.PC] + 'MQL4/Files/' + ea + '/Test_*/*.htm')
                 if 'WheSitDoch' == ea else instance.sort_paths(True))
        ...
        try:
            for i in range(0, len(tests)):
                ...
                # 進捗表示
                if 'WheSitDoch' == ea:
                    bar1 = tests[i].replace('\\', '/').split('/')[-2]
                    bar2 = tests[i].replace('\\', '/').split('/')[-1].split('.')[0]
                else:
                    bar1 = tests[i][0].replace('\\', '/').split('/')[-2]
                    bar2 = tests[i][0].replace('\\', '/').split('/')[-1].split('.')[0]
                ...
                for k in range(0, len(tests[i])):
                    ...
                    try:
                        # HTMLのヘッダー部から取得
                        datas = open(test, 'r', encoding=('utf16' if 'WheSitDoch' == ea else 'cp932')).read()
                        ...
                        if 'WheSitDoch' == ea:
                            # 初期資金
                            balance_start = float(100000 * (10 if targets[-1].startswith('XAUUSD') else 1))
                            dd_max = datas[datas.find('最大ドローダウン'):]
                        ...
                        # HTMLの時系列データを取得
                        datas = pd.read_html(test, encoding=(
                            'utf16' if 'WheSitDoch' == ea else 'cp932' if 'Win' == cst.PC else 'utf8'),
                        ...
                        for n in range(0, len(datas)):
                            if not str(datas.at[n, '取引種別']).startswith('buy') \
                                    or not str(datas.at[n, '取引種別']).startswith('sell') \
                                    or not str(datas.at[n, '取引種別']).startswith('Buy') \
                                    or not str(datas.at[n, '取引種別']).startswith('Sell'):

                                start_year = datas.at[n, '時間'][:4]
                                start_balance = (0 if 'nan' == str(datas.at[n, '残高']) else datas.at[n, '残高'])
                                break
                        ...
                        for m in range(0, len(datas)):
                            ...
                            n = m
                            if m == len(datas) - 1:
                                while 'nan' == str(datas.at[n, '残高']):
                                    n -= 1
                                last_balance = datas.at[n, '残高']

                            if 'nan' != str(datas.at[n, '残高'])  \
                                    and (not str(datas.at[n, '取引種別']).startswith('buy')
                                         or not str(datas.at[n, '取引種別']).startswith('sell')
                                         or not str(datas.at[n, '取引種別']).startswith('Buy')
                                         or not str(datas.at[n, '取引種別']).startswith('Sell')):
                                ...
                                # 年率計算用
                                if ('nan' != str(datas.at[n, '時間']) and start_year != datas.at[n, '時間'][:4]) \
                                        or m == len(datas) - 1:
                        ...
                        # 名前と表示クラスの定義
                        ea_name = (targets[-1].split('.')[0] if 'WheSitDoch' == ea else ea_name
                            if 1 == len(cst.EA_PATHS[ea_name.split('_')[0]]) else targets[-1].split('.')[0])
                        ea_class = ('ea_all' if 0 <= ea_name.find('Full') or 0 <= ea_name.find('Best') else 'prm')

                        # 推奨ロット計算と、必要なアイテムの計算
                        lot = lot / (balance_start / 10000) * \
                              (10 if 'WheSitDoch' == ea and targets[-1].startswith('XAUUSD') else 1)
                        ...
                        if 'WheSitDoch' != ea:
                            html_row += '<font color="#000000">(' + str(win_max) + ')</font>'
                        html_row += '</td><td class="' + ea_class + ' warning" align="right">' + str(lose)
                        if 'WheSitDoch' != ea:
                            html_row += '<font color="#000000">(' + str(lose_max) + ')</font>'
                        html_row += '</td>'
