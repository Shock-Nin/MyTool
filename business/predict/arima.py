...
def create(dict_df, inputs):
    ...
    fig, ax = plt.subplots(1, len(dict_df), figsize=(16, 5))
    ...
    try:
        for currency in dict_df:
            ...
            window = com.progress('LSTMモデル作成中', [currency, len(dict_df)], interrupt=True)
            ...
            ax[cnt].plot(train['Close'])
            ax[cnt].plot(valid[['Close', 'Predictions']])
