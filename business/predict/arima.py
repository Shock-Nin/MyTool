...
def run(currency, df, forecast, interval, str_pdq, lag, loaded_result=None, ar_only=False):
    ...
    try:
        ...
        if loaded_result is None:
            ...
            for i in range(split_len, len(df_all), interval):
                ...
                refit_summaries[0].append(refit_summary[0])
                refit_summaries[1].append(refit_summary[1])
                refit_summaries[2].append(refit_summary[2])
                refit_summaries[3].append(refit_summary[3])
                actual_summaries[0].append(actual_summary[0])
                actual_summaries[1].append(actual_summary[1])
                actual_summaries[2].append(actual_summary[2])
                actual_summaries[3].append(actual_summary[3])
            ...
            refit_summaries = [
                float(round(np.mean(refit_summaries[0]), 5)), float(round(np.mean(refit_summaries[1]), 5)),
                float(round(np.mean(refit_summaries[2]), 5)), float(round(np.mean(refit_summaries[3]), 5)),
                round(sqrt(mean_squared_error(predict_data['test'], predict_data['refit'])), 5)]

            actual_summaries = [
                float(round(np.mean(actual_summaries[0]), 5)), float(round(np.mean(actual_summaries[1]), 5)),
                float(round(np.mean(actual_summaries[2]), 5)), float(round(np.mean(actual_summaries[3]), 5)),
            ...
            model_summaries = [
                float(round(np.mean(model_summary[0]), 5)), float(round(np.mean(model_summary[1]), 5)),
                float(round(np.mean(model_summary[2]), 5)), float(round(np.mean(model_summary[3]), 5)),
                round(sqrt(mean_squared_error(predict_data['test'], predict_data['normal'])), 5)]

            str_summary = 'Type: [Normal, Refit, Predict]\n' \
                + f'AIC: [{model_summaries[0]}, {refit_summaries[0]}, {actual_summaries[0]}]\n' \
                + f'BIC: [{model_summaries[1]}, {refit_summaries[1]}, {actual_summaries[1]}]\n' \
                + f'HQIC: [{model_summaries[2]}, {refit_summaries[2]}, {actual_summaries[2]}]\n' \
                + f'BSE: [{model_summaries[3]}, {refit_summaries[3]}, {actual_summaries[3]}]\n' \
                + f'RMSE: [{model_summaries[4]}, {refit_summaries[4]}, {actual_summaries[4]}]'
            print(str_summary)
        ...
        msg = ', '.join([msg for msg in [
            (f'ARIMA: ({str_pdq.replace(',', ' ')})' if not ar_only and 3 == len(pdq) else ''),
            f'AR({str(lag)})'] if '' != msg])
        ...
        if not ar_only and 3 == len(pdq):
            # ax1.fill_between(np.arange(split_len, split_len + len(forecast_data['arima']), 1),
            #                  # forecast_data['forecast_upper'], forecast_data['forecast_lower'],
            #                  color='orange', alpha=0.2)
            ax1.plot(forecast_data['arima'], label='ARIMA', linewidth=1, color='green', alpha=0.5)
...
def _predict_model(df, arima_type, prm, cnt, interval):

        model = _create_model(df, arima_type, prm)
        prediction = model.predict(cnt, cnt + interval - 1)
        summary = [model.aic, model.bic, model.hqic, model.bse]
        model.remove_data()

        return prediction, summary

def _create_model(df, arima_type, pdq):
    try:
